...
void bridge_tree_dfs_simple(ll v, ll total_vertices, vector<bool> &visited, vector< vector <ll> > &total_neighbours, vector<ll> &component, ll &component_number){
 	visited[v]= true;
...
void find_bridge_tree_simple(const ll total_vertices, vector< vector <ll> > &total_neighbours, vector<ll> &bridge_tree_map, 
...
void enhanced_bridge_tree_dfs(ll v, ll total_vertices, vector<bool> &visited,  
			vector<vector<tuple<ll, ll, bool>>> &enhanced_total_neighbours, vector<ll> &component, ll &component_number){
 	visited[v]= true;
	component[v]= component_number;
...
void find_bridge_tree(const ll total_vertices, vector<vector<tuple<ll, ll, bool>>> &enhanced_total_neighbours, vector<ll> &bridge_tree_map){
	ll k= 0;
	vector <bool> visited(total_vertices, false);
	bridge_tree_map = vector<ll>(total_vertices, 0);
	for (int i= 0; i< total_vertices; i++) {  
...
void dfs_find_bridges(const ll v, vector<ll> &pre, vector<ll> &low, vector<bool> &visited, vector<pair<ll, ll>> &edges, 
		vector<ll> &parent, ll total_vertices, ll &time, vector<vector<tuple<ll, ll, bool>>> &en_adj){
	visited[v]= true;
	time++;
	low[v]= time;
	pre[v]= time;
...
void dfs_find_bridges_simple(const ll v, vector<ll> &pre, vector<ll> &low, vector<bool> &visited, unordered_set<pair<ll, ll>, hashFunction> &total_bridges, 
		vector<ll> &parent, ll total_vertices, ll &time, vector<vector<ll>> &total_neighbours){
	visited[v]= true;
	time++;
	low[v]= time;
	pre[v]= time;
...
void find_bridges_simple(const ll total_vertices, vector<vector<ll>> &total_neighbours, unordered_set<pair<ll, ll>, hashFunction> &total_bridges){
	vector<ll> pre(total_vertices, int(0));
	vector<ll> low(total_vertices, int(0));
	ll component_number= 0;
	vector<ll> parent(total_vertices, int(-1));
